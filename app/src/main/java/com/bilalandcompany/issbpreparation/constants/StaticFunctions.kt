package com.bilalandcompany.issbpreparation.constants
...
object StaticFunctions {
    fun inviteOthers(context: Context?) {
        ...
        sendIntent.putExtra(Intent.EXTRA_SUBJECT, "OgreSpace Invitation")
        sendIntent.putExtra(
            Intent.EXTRA_TEXT, "Hi, I want to invite you to join me to give OgreSpace, A Super-Cool Property App.\n\n" +
                    "https://play.google.com/store/apps/details?id=com.brainplow.ogrespace"
        )
    ...
    fun loadImage(context: Context?, image:String?, imageView: CircleImageView?){
        Glide.with(context!!)
            .load(image).placeholder(R.drawable.cplaceholder)
            .into(imageView!!)
    }
    fun loadImage(context: Context?, image: Uri?, imageView: ImageView?){
        Glide.with(context!!)
            .load(image).placeholder(R.drawable.cplaceholder)
            .into(imageView!!)
    }
    ...
    fun loadImage(context: Context?, image:String?, imageView: CircleImageView?, storage:String){
        Glide.with(context!!)
            .load(storage+image).placeholder(R.drawable.cplaceholder)
            .into(imageView!!)
    }
    fun checkNumberValidation(number:String):Boolean{
        ...
        val regex = "^(\\([0-9]{3}\\)|[0-9]{3}-)[0-9]{3}-[0-9]{4}\$\n"
